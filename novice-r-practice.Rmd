...
```{r read-measurements}
measurements <- readr::read_csv(here::here("novice-r-practice", "measurements.csv"))
```
...
```{r join-two-tables}
cleaned <- readr::read_csv(here::here("novice-r-practice", "visited.csv")) %>%
...
```{r}
hra <- readr::read_csv(here::here("novice-r-practice", "home-range-database.csv"))
head(hra)
```
...
hra %>%
  ...
  ggplot2::geom_smooth(method = lm, mapping = aes(x = log10.mass, y = log10.hra), color = 'red', se = FALSE)
```
