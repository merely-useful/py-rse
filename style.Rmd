...
-   The file's documentation string (Chapter \@ref(s:style)).
...
for logging purposes (Chapter \@ref(s:logging)).
...
extract a function (Section \@ref(s:refactor-extract-function)).
...
Section \@ref(s:refactor-short-circuits)
...
-   The commented-out `print` statements should either be removed
    or turned into proper logging statements (Chapter \@ref(s:logging)).
...
:   Code review isn't just (or even primarily) about style:
    ...
    Are errors being caught and handled Chapter \@ref(s:logging)?
...
1.  Figure out what the user has asked it to do (Chapter \@ref(s:configure)).
...
One solution is to give parameters default values (Chapter \@ref(s:style));
