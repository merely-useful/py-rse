...
```{r r-repro-blank-slate, eval=FALSE}
...
```{r r-repro-load-package-example, eval=FALSE}
library(tidyverse)
```
...
TODO: Move this below readability maybe?
...
So instead of manually inserting a figure for example, you write R code within
the document to insert the figure for you! Using [R Markdown][r-markdown] can
save so much time and make your work that much closer to being reproducible.
...
document types. There are many "flavours" of Markdown that have been developed
and R Markdown uses is the [Pandoc][pandoc-markdown] version.

Markdown is a "markup language" (like HTML, but simpler) meaning that special
characters mean certain things, which we will cover below. Write Markdown as
plain
text (like R code) 
...
Choose the HTML output format. Save this file as `rmarkdown-session.Rmd` in the
`doc/` folder.
