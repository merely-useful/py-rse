# Continuous Integration {#s:integrate}
...
so that people only create pull requests for things that are working (Chapter \@ref(s:branches)).
...
those tests can check the code's style as well as its correctness (Chapter \@ref(s:style)).
...
like many systems,
Travis puts its configuration file in the project's root directory (Chapter \@ref(s:configure)),
...
by convention,
we put a list of packages in `requirements.txt` for `pip` to use (Chapter \@ref(s:package)).
Finally,
...
FIXME: forward ref to Section \@ref(s:tools-cron).
