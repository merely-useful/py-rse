...
and starting from your default environment with `--clone root`:

```bash
$ conda create -n pyzipf --clone root
```

```text
Source:      /Users/amira/anaconda3
Destination: /Users/amira/anaconda3/envs/pyzipf
...
```text
/Users/amira/anaconda3/bin/python
```
...
Let's make some additions that will reliably automate
the process of installing all dependencies for our package
...
Next,
we can install our package using the `setup.py` file:
...
Requirement already satisfied: matplotlib in /Users/amira/anaconda3/envs/pyzipf/lib/python3.7/site-packages (from pyzipf==0.1) (3.3.2)
...other requirements...
...
Installing collected packages: pyzipf
  Attempting uninstall: pyzipf
    Found existing installation: pyzipf 0.1
    Uninstalling pyzipf-0.1:
      Successfully uninstalled pyzipf-0.1
  Running setup.py develop for pyzipf
Successfully installed pyzipf
```

The output looks generally the same as the first install
because `pip` could again re-use packages saved locally
...
we would have to manually uninstall it before reinstalling it during development.)
...
```bash
(pyzipf)$ conda create -n pyzipf-test --clone pyzipf
...
matplotlib in /usr/lib/python3.7/site-packages
(from pyzipf) (3.2.1)
...other requirements...
Installing collected packages: pyzipf
