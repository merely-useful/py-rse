...
```{r git-advanced-dracula-fit, echo=FALSE, fig.cap="Word frequency distribution for Dracula."}
...
```text
[fit efdbb24] Added fit to word count data
...
```text
efdbb24 (HEAD -> fit) Added fit to word count data
0da7260 (master) removing inverse rank
```
...
```text
0da7260 (HEAD -> master) removing inverse rank
d18d401 ignoring __pycache__
```
...
```text
[fit b0b6115] Adding Moreno-Sanchez et al (2016) reference
...
```text
Updating 0da7260..b0b6115
Fast-forward
 bin/plotcounts.py   |  82 ++++++++++++++++++++++++++++++++++++++
                           ++++++++++++++++++++++++++++++++++++++
                           ++++++
...
```text
b0b6115 (HEAD -> master, fit) Adding Moreno-Sanchez et al (2016) reference
efdbb24 Added fit to word count data
0da7260 removing inverse rank
d18d401 ignoring __pycache__
```

Note that Git automatically creates a new commit (in this case, `b0b6115`)
...
```text
Deleted branch fit (was b0b6115).
```
...
```text
[master 8afd72f] Initial commit of README file
...
```text
[docs cfcf5e5] Added repository overview
 1 file changed, 2 insertions(+)
```
...
```text
[master 83e9077] Added contributor list
...
```text
[master 289b8e3] Merging README additions
```
...
```text
289b8e3 (HEAD -> master) Merging README additions
83e9077 Added contributor list
cfcf5e5 (docs) Added repository overview
8afd72f Initial commit of README file
```
...
```text
Deleted branch docs (was cfcf5e5).
```
...
remote: Total 64 (delta 19), reused 64 (delta 19), pack-reused 0
Receiving objects: 100% (64/64), 2.20 MiB | 2.99 MiB/s, done.
...
```text
289b8e3 (HEAD -> master, origin/master, origin/HEAD) 
        Merging README additions
83e9077 Added contributor list
cfcf5e5 Added repository overview
8afd72f Initial commit of README file
```
...
```text
[master 7945489] Adding Sami as a contributor
...
Unpacking objects: 100% (3/3), 296 bytes | 74.00 KiB/s, done.
From https://github.com/amira-khan/zipf
 * branch            master     -> FETCH_HEAD
 * [new branch]      master     -> upstream/master
Updating 289b8e3..7945489
Fast-forward
 README.md | 1 +
 1 file changed, 1 insertion(+)
```
...
```text
[adding-email 2a4ac8c] Adding my email address
