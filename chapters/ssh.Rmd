...
```{r ssh-ssh, echo=FALSE, fig.cap="To work remotely, the local client connects to the remote login server, and if successful, the remote login server starts a remote shell (solid lines). Once logged in, commands issued in the local client are passed on to the remote shell, and any output is passed back to the local client (dashed lines)."}
insert_graphic("figures/ssh/ssh.pdf")
```
