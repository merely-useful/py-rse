...
1. It is difficult to write a simple unit test for the lines of code dedicated to reading input files,
because `main` is a long function that requires command line arguments as input.
Edit `collate.py` so that the six lines of code responsible for processing an input file
...
4. Use the `coverage` library to check that the relevant commands in `process_file`
(specifically `raise OSError` and `open(file_name, 'r')`) were indeed tested.
