...
Our code will inevitably contain bugs,
and people will misunderstand how to use our programs correctly.
To resolve these issues as effectively as possible
we should plan from the start to catch and handle errors in our programs.
In this chapter,
we will explore how such errors are represented
and what we should do with them.
Before diving into the details of each section below,
it is helpful to have an overview of how the error handling process.

When a program encounters an error during execution
it is often referred to as an *exception* from the normal behavior.
Such errors can occur for various reasons and in this chapter,
we will see the difference between *internal* and *external* errors.
An example of and internal error would be an unsupported option being passed to a function,
and an external error could be trying to read a file that does not exist.
Once an exception is encountered,
it is important that the program communicates the cause of the error
via an informative error message,
so that the user can take the appropriate action to fix it.
Not all errors can be resolved immediately,
so it is important to keep a log of the error messages.
Logging error messages to a file facilitates troubleshooting
because it is easier to trace back what has gone wrong,
and also to share the full error report with others.
