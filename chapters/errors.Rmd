...
\gref{External errors}{external_error} are usually caused by interactions
between the program and the outside world:
a user may mis-type a filename,
the network might be down,
and so on.

When a program encounters an internal error,
we'd typically like it to stop immediately
and provide an informative error message.
Unfortunately, one or both of those things doesn't always happen.
The default error messages for many packages can be very cryptic (especially for non-experts),
and they can be even more confusing if they aren't triggered right away
but instead two or three or four steps down the track.
For external errors,
we often don't want the program to stop when an error occurs.
If a user mis-types her password,
prompting her to try again would be friendlier than
requiring her to restart the program.

This chapter looks at how we can catch, classify and handle errors.
We consider how to write useful error messages,
and how to make our programs log information as they are running
so that when something goes wrong we can figure out what it was.
