...
takes in a string of letters and then prints them back out to the console. 
...
We've now told the computer what to do by using Python code. We've run that 
...
our analysis. We save that information by giving it a name (called a *variable*), 
...
of information - `str` (string), `int` (integer), `float` (decimal point 
number) and more. 

## Getting Help

However, that top-right pane can also give us additional information; one of 
...
Typing the full name of the function with the Help tab selected will also 
...
computer's hard drive or "disk". This is a componenent of the computer that
...
contained in the file on disk in the computer's *memory*. Memory is a temporary 
...
Anything saved on disk is placed somewhere in the computer's "filesystem" - 
a hierarchy of folders, which in programming-speak, are called directories. 
Because directories (folders) are nested, they are sometimes drawn as branching 
diagrams - another name for a file system is "directory tree". 
