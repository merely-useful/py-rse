...
come in from a few sources.  Note that you will sometimes hear packages refers 
...
Certain functionality that is considered essential for the Python programming language is available wherever Python is installed. This is the Python Standard Library, and is the language content maintained by the core language team (PEP-0013).  Anything you see within the https://docs.python.org/ pages is part of the stardard library.  Other highly useful, but often more domain specific functionality can be accessed separately in the
...
1. Import the entire package via `import <package_name>`, giving you access to the 
package content by way of the package's name. E.g. `<package_name>.<function_name>()`.
2. Import just content that you want, but directly into your current namespace via 
`from <package_name> import <function_name>`. This gives you access to that content 
...
The `import <package_name>` syntax is the most common and generic method of importing 
packages, but there are several other ways to do this. 
...
np.mean([1, 2, 3])
```
Using this syntax makes `numpy` not defined within that code. 
