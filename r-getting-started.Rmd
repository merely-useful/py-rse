...
## Objectives {#r-getting-started-objectives}
...
RStudio (like many IDEs) opens up and has many panes, that each have a different
purpose.
...
R has many built-in functions, that allow you to do different things:

```{r}
length(c(4, 6, 3, 20, 100))
sort(c("Samuel", "Nathaniel", "Augustus", "Tracy"))
min(c(4, 2/3, -3, 6.25))
```
...
we'll have to use various run commands. RStudio includes a "Run" button in the
script pane, but typically you'll want to learn a few keyboard shortcuts
to make it quicker and easier to run code written in a script.
easy to run code that's written in a script. Try "running"
You should see their output in the console on the
bottom right.
...
programming language as an [object](glossary.html#object). There are different
...
Most types have special functions specific to that type, called [methods](glossary.html#method). Methods look like functions (`name()`), but instead
...
## Getting Help in RStudio {#py-getting-started-spyder-help}
