...
in the final version, but you want to be able to refer back to. You may want to 
work on a document and then start over if it's not going well. 
It can be nice to have a record of changes you've 
made and the previous versions of your work just for your own emotional reassurance. 
...
The multiple-file systems above is not ideal though! 
It requires making multiple copies of the 
same file, even if the changes are isolated to one part of the file. It also 
isn't always easy to know which previous file has the information that you want 
to retrieve. 

Luckily, there's a better solution - using a version control system. A version 
control system is a specific software tool that can be used to track changes 
...
information about the state of our `carbon.py` file in the repository we created 
in the previous section. Now, if we make additional changes to the `carbon.py` 
file, we'll be able to create new commits, building up a history of changes. Try 
making changes to `carbon.py` and committing them. Create a `README` file and 
commit that as well. The best way to get used to the process of using version 
...
make a change to the `carbon.py` file in your Github repository. You can do this 
...
TODO: how to do this in GitKraken. Blah. 
