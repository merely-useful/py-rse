...
our recommendations often refer to the organization methods
suggested in Section \@ref(rse-project) as well as tools from GitHub
that integrate with material from Section \@ref(rse-git-advanced)
...
As project participants, we should
As project leaders, we should specify and enforce expectations
to encourage an inclusive culture and teach participants to be allies.

## Why should we establish a code of conduct for a project? {#rse-teams-coc}

FIXME: adding positives (curb cuts), removing negatives
...
A Code of Conduct is implemented to promote equity and fairness among a group of partcipants.
FIXME
We recommend using the [Contributor Covenant][covenant] for online development projects
...
As discussed in \@ref(rse-project-boilerplate),
...
It might seem strange to include licensing in a discussion managing teams and projects,
...
[issues][issue] tell us where we're going.
...
### Agile Planning
Agile is a project managing style that is based on the idea of simultaneous working on different aspects of the project (such as research, design and development) and deliver a product (e.g. a self-contained working piece of software) over a finite time interval (called sprint). The consecutive iterations are delivered based on continuous update of the product (based on usage feedback) and integrating all the pieces of the final software. For agile planning you need to
- define deliverables,
...
what you have to do:
step 1. gather the team, either physically or online
step 2. write the breakdown of the project tasks with the team
step 3. draw a 3x3 grid and label the axes as 'impact' and 'effort'; each having 3 levels of low, medium, and high for grid cells.
step 4. have the team place each item on the grid based on the level of impact and effort
step 5. prioritize and delegate the tasks (or eliminate if needed!)
