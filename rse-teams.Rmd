...
and will need to be asked for approval when a license *is* chosen. Silimarly problematic, is that changing a license does not change it retroactively. So you may end up with multiple users operating under different licesning structures. 
...
where the "0" stands for "zero restrictions".  This is the public demain declaration, and is not actually a license.
...
-   The issue's current status.
    ...
    and by default,
    only open issues are displayed. Closed items are genearlly removed from default interfaces, so issues should only be closed when they can be completely removed from the team's attention.
...
and the more likely the response will actually address the issue @Bett2008. Sometimes the person creting the issue may not know or have the right answer for some of these things, and will be doing their best with limited informatio about the error. Responding with kindness and encouragement is important to maintain a healthy community, and should be enforced by the project's code of conduct.
...
-   *Won't Fix*: someone has decided that the issue isn't going to be addressed,
    ...
    it is usually then closed. This is a great opportunity to provide an kind note expalaining why it won't be addressed and encouragement to continue working with the project. 
...
We can leverage our issue tracking system as described in Section \@ref(rse-teams-labels)
...
and management in a `CONTRIBUTING` document,
as described in \@ref(rse-project-boilerplate),
so newcomers (and existing participants!)
know what to expect.

## How can we make decisions as a group? {#rse-teams-martha}
...
and as we learned in Section \@ref(rse-teams-inclusive),
...
both in terms of collaborator guidelines (see \@ref(rse-teams-issues)),
as well as tracking progress (see \@ref(rse-teams-workflow)).
...
## How can we handle conflict to support an inclusive culture? {#rse-teams-conflict-inclusive}
...
### Responding to conflict about project priorities {#rse-teams-ex-priority-conflict}
