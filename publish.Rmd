...
That means publishing it in an open access venue (Chapter \@ref(s:inclusive))
...
-   Give every dataset and every report a unique identifier (Section \@ref(s:publish-identifiers)).
-   Put data in open repositories (Section \@ref(s:publish-data)).
...
1.  (Meta)data is assigned a globally unique and persistent identifier (Section \@ref(s:publish-identifiers)).
...
4.  (Meta)data is registered or indexed in a searchable resource,
    such as the data sharing platforms described in Section \@ref(s:publish-data).
...
using the conventions described in Chapter \@ref(s:project).
...
Be sure to give the dataset an identifier as discussed in Section \@ref(s:publish-identifiers).
...
2.  Go back to that GitHub repository and go to the "Releases" tab.
    ...
    give it a version number (discussed in Section \@ref(s:branches-tag)),
...
and your scripts almost certainly depend on specific versions of other packages (Chapter \@ref(s:package)).
...
As Section \@ref(s:package-install) described,
...
For everything else,
you should write a script or create a rule in your project's Makefile (Chapter \@ref(s:automate)),
...
If all of the program's parameters are in a configuration file (Chapter \@ref(s:configure)),
you can archive that.
Otherwise,
have your program print out its configuration parameters
and use `grep` or a script to extract from the logfile (Chapter \@ref(s:logging)).
