...
modern publishing involves much more than a written report.
It involves also making details of the data, software and code underpinning
the written report available to readers.
...
The report simply needs to document where to access the dataset
and what version was analyzed,
along with any software and code used to process the data (see next section).
In other words, it's not necessary to re-publish a duplicate of the original dataset
if it's already accessible elsewhere.

Strictly speaking, it wouldn't be necessary to publish any data files
that were produced during the analysis either,
since readers have access to the original data and the software/code used to process it.
Having said that, it can be advantageous to published processed data that difficult to reproduce
...
(i.e. something that people might want to pick up and apply in their own work).
For instance, it would be worthwhile to publish 
an estimate of the global average surface temperature derived from
a large database of weather observations around the globe,
because a simple metric of global warming could be useful in many subsequent studies.

If a report involved the generation of a new dataset
(e.g. ocean temperature observations collected on a research voyage),
then clearly that dataset would need to be published.
...
## Software and code {#publish-software}

TODO: Add intro explaining difference between software and code and the components of the
computation process that need to be shared:
