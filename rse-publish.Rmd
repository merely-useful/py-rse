...
In many cases,
archiving data at a site like Dryad or Figshare (following the FAIR Principles)
is the end of the data publishing process. 

For datasets with high re-use potential,
there is however the option of publishing an article in a data journal
to describe the strengths, limitations and applications of the dataset.
Given the power of data to aid in decision‐making
and the specialised nature of many fields of research,
simply letting a dataset loose without an explanation of its development
or initial purpose may result in unintentional misuse.
Journals such as the
[Geoscience Data Journal](https://rmets.onlinelibrary.wiley.com/journal/20496060)
and [Scientific Data](https://www.nature.com/sdata/)
have been established to address this problem.
These journals don't archive the dataset itself,
but they do have strict guidelines (often similar to the FAIR Principles)
on where and how the datasets are archived.
An added benefit for those working in academia is that data users 
can cite the paper from the data journal when they use the dataset it in their own research.
This ensures that the authors receive the academic credit (i.e. journal paper citations)
they deserve for the hard work associated with creating and maintaining a dataset.
...
Creating a DOI using a site like Zenodo is often the end of the software publishing process.
Similar to data journals, there is however the option of going a step further and publishing
a journal paper devoted to describing the software in detail.
...
Importantly, in order to be published with one of these journals the software
should be a significant contribution to the available open source software 
that either enables some new research challenges to be addressed
or makes addressing research challenges significantly better (e.g., faster, easier, simpler).
