...
2.  `bin/countwords.py` reads a text file and creates a CSV file with two columns:
    ...
    We can analyze several files at once by [piping][unix-pipe] them into the program
    using something like `cat data/*.txt | bin/countwords.py`.
    (If you don't know what this means,
    now would be a good time to review the material on the Unix shell in Chapter \@ref(rse-bash).
...
2.  A user can describe which files depend on each other
    by writing [rules][make-rule] in a [Makefile][makefile].
    For example,
...
The second and third lines form a [rule][make-rule]:
the [target][make-target] of the rule is `results/moby_dick.csv`,
its single [prerequisite][make-prerequisite] is the file `data/moby_dick.txt`,
and the two are separated by a single colon `:`.

The target and prerequisite tell Make what depends on what;
the indented line below them describes the [action][make-action] needed
...
which is called the [default target][make-default-target].
In this case,
...
The key is to create a [phony target][make-phony-target]
...
then brings each of those prerequisites up to date.
...
define and use [variables][make-variable].
...
To do this,
we need to understand Make's [automatic variables][make-automatic-variable].
...
We can now replace all the rules for generating results files with one [pattern rule][make-pattern-rule]
...
We can use a [function][make-function] to do this.
...
and `grep` (Section \@ref(rse-bash-finding)) to pull these lines out of the file:
...
FIXME: create summary file and plot.

-   Point out that using pattern substitution is better than direct shell wildcards like `results/*.csv`
    because the latter gives false positives for downstream files.
