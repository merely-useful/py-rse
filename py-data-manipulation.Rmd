...
Since only `rstrip()` actually combines any values, we'll just do that.

```python
# strip whitespace from the end of each borough name
...
Use `groupby()` and `agg()` with the dog license data frame `data` after adding the `AgeWhenLicensed` column to find the mean, min, and max age when a dog was licensed in days grouped by `Borough`. Hint: the pandas `timedelta` format of the `AgeWhenLicensed` column will make calculating numeric quantities difficult; try creating a new column for the licensed age in days using `data['AgeWhenLicensed'].dt.days`.
