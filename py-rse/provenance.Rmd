...
4.  (Meta)data is registered or indexed in a searchable resource,
    such as the data sharing platforms described in Section \@ref(publish-where).
...
(e.g. matplotlib, numpy) to produce the key figures and results.
...
the bare minimum requirement would be for us to archive a list of
...
but we would be leaving it up to the reader of the report
to figure out how to get all those software packages
installed and working together.
In some cases this might be relatively straightforward,
but in other cases we might want to make life easier for the reader
(and for our future selves looking to re-run our analysis).
For example,
...
Later in the book we will package and release our Zipf's Law code
so that it can be downloaded, installed and used by the wider research community
...
but often the scripts and notebooks you write
to produce a particular figure or table in a report
will not be of broad interest.
They were written to get a specific job done (i.e. to produce that figure)
and do not have a wider application.
In order to fully capture the provenance of the major figures/results in a report,
...
FIXME: Talk about Figshare and Zenodo.
