...
We'll be continuing to work in `zipfs-law/`,
...
Let's start by ensuring our working directory is `zipfs-law`,
...
and we can check its contents using `cat book_summary.sh`.
More importantly,
...
The command `grep` searches for every line that contains a pattern and prints the matching lines (we will learn more about `grep` later).
...
We should probably also change the script's name from `book_summary.sh` to `book_short_summary.sh`,
...
FIXME: include these files in data/,
indicate where they can be downloaded,
or update to zipfs-law examples
...
so if we want to count how many lines in our books contain the word pain, we can do this:
...
we will explore the `docs` directory within `zipfs-law`:

FIXME: include these files in the original data download,
or update examples
...
FIXME: uses same data from previous chapter,
update as per decision there
...
### Tracking a species {#py-rse-bash-advanced-ex-tracking-species}
...
### Counting names {#py-rse-bash-advanced-ex-little-women}

FIXME: update to a novel for which we still have data?
