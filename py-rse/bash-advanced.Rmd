...
we will use the entire contents of our `zipfs-law` directory,
...
### Counting names {#py-rse-bash-advanced-ex-sense_sensibility}

You and your friend,
having just finished reading *Sense and Sensibility* by
Jane Austen, are in an argument.  
...
We've discussed how options can be combined into a single unit
(for example,
`ls -F -a` is equivalent to `ls -Fa`).
Thinking about the commands we've learned in this chapter,
under what circumstances could you imagine it would be
*not* possible or advisable to combine options?
