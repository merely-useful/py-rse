...
-  Create a configuration file to set plotting options in `plotcounts.py`.
-  Add some new command-line options to `plotcounts.py` .

Let's consider these options one by one.

## The Global Configuration File {#configuration-global}
...
We can uncomment those lines change the sizes to "large" and "extra large":
...
## Creating a Style Sheet {#configuration-stylesheet}
...
## Adding Command-Line Options {#configuration-command-line}
...
## A Job Control File {#configuration-job-file}
...
And if we need to manage differences between different installations of the software,
user-specific and system-wide configuration files are probably the best choice.
...
### Making plots more accessible {#configuration-ex-accessible}
...
### Saving configurations {#configuration-ex-debug}
