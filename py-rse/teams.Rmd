...
Create a new file called `CONDUCT.md` in `zipfs_law/` (root directory).
...
> as discussed in Chapter \@ref(py-rse-git-advanced).
...
In Chapter \@ref(py-rse-errors), we discussed adding exceptions to `collate.py`.
...
both in terms of collaborator guidelines (Section \@ref(py-rse-teams-coc)),
as well as tracking progress (Section \@ref(py-rse-teams-workflow)).
