...
covered in Chapter \@ref(py-rse-project) as well as tools from GitHub
as described in Chapter \@ref(py-rse-git-advanced).
...
FIXME: Chapter on error handling modified either: `countwords.py`, `collate.py` or `plotcounts.py`
FIXME: This exercise should involve one of the other two scripts
In Chapter \@ref(py-rse-errors), we added exceptions to `countwords.py`.
In your GitHub repository, file an issue representing a bug report
that includes information to resolve a similar issue in one of the other scripts.

It takes time and energy to write a good error report.
If the report is being filed my a member of the development team,
...
FIXME: summarize py-rse-teams, concept map?

## Exercises {#py-rse-teams-exercises}

### pandas scavenger hunt {#py-rse-teams-ex-scavenger-hunt}
