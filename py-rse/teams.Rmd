# Working in Teams {#py-rse-teams}

```{r py-rse-teams-setup, include=FALSE}
...
covered in Section \@ref(py-rse-project) as well as tools from GitHub
as described in Section \@ref(py-rse-git-advanced).

## Why is it important to ensure everyone feels welcome to contribute? {#py-rse-teams-inclusive}
...
## How do we establish a code of conduct for a project? {#py-rse-teams-coc}
...
Using a text editor, outline a few essential elements for
the Code of Conduct and its enforcement for your Zipf's Law project.
Name the file `CONDUCT` and save it in your project directory.
...
## Why should we include a license? {#py-rse-teams-license}
...
### How should we license our software? {#py-rse-teams-license-software}
...
Similar to `CONDUCT` described above,
the license is usually put in clearly labeled file in the project's root directory.
In a text editor, create a file called `LICENSE`
...
### How should we license our data and reports? {#py-rse-teams-license-other}
...
perhaps on Mary Shelley's classic book Frankenstein.
...
> as discussed in Section \@ref(py-rse-git-advanced).

## How can we keep track of the work we still have to do? {#py-rse-teams-planning}
...
In Section \@ref(#py-rse-teams-coc),
...
## How can I write good bug reports? {#py-rse-teams-bugs}
...
and should be enforced by the project's Code of Conduct (Section \@ref(#py-rse-teams-coc)).
...
6.  Describe each problem separately so that each one can be tackled on its own.
    ...
    discussed in Section \@ref(py-rse-git-advanced).
...
EXERCISE: add issue for bug report (for something that will be resolved later?)

## How can we use labels to organize work? {#py-rse-teams-labels}
...
-   *Discussion* or *Proposal*: something the team needs to make a decision about
    ...
    as we'll discuss in Section \@ref(py-rse-teams-martha),
...
EXERCISE: create/modify labels in your GitHub project
Labels: Bug, Assigned (for bug report above)

## How do we decide the priority of certain tasks? {#py-rse-teams-prioritize}
...
## How do we standardize a workflow for the project? {#py-rse-teams-workflow}
...
We can leverage our issue tracking system as described in Section \@ref(py-rse-teams-labels)
...
EXERCISE: We should describe the systems we're using for project organization
and management in a `CONTRIBUTING` document,
as described in \@ref(py-rse-project-boilerplate),
so newcomers (and existing participants!)
know what to expect.

## How can we make decisions as a group? {#py-rse-teams-martha}
...
and as we learned in Section \@ref(py-rse-teams-inclusive),
...
## How can we run meetings more efficiently? {#py-rse-teams-meetings}
...
## How can we keep people from talking too much or too little? {#py-rse-teams-fair}
...
## How should we run online meetings? {#py-rse-teams-online}
...
## How should we handle conflict within the team? {#py-rse-teams-conflict}
...
both in terms of collaborator guidelines (see \@ref(py-rse-teams-issues)),
as well as tracking progress (see \@ref(py-rse-teams-workflow)).
...
## How can we handle conflict to support an inclusive culture? {#py-rse-teams-conflict-inclusive}
...
## Summary {#py-rse-teams-summary}

FIXME: summarize py-rse-teams
highlight contributing guidelines, readme, CoC, etc
create concept map

## Exercises {#py-rse-teams-exercises}

### ggplot2 contributing scavenger hunt {#py-rse-teams-ex-scavenger-hunt}

FIXME: replace ggplot with python example

Take a look at the ggplot2 GitHub repository. Where is the information for license, contributing, etc?
https://github.com/tidyverse/ggplot2

### Responding to conflict about project priorities {#py-rse-teams-ex-priority-conflict}
...
### Use of Wiki feature in GitHub to aggregate FAQs
