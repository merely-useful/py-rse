...
r = cf^{\frac{-1}{\alpha}}
\]
Then the linear relationship between the log word frequency and 
log rank is \[
\log(r) = C - \frac{1}{\alpha}\log(f)
\]
This suggests, on our log-log plot, 
our points should fall on a straight line 
with a slope of \(- \tfrac{1}{\alpha}\) 
and intercept \(C\) (we'll see later that \(C\) is completely defined once we know \(\alpha\).
