...
> The `zipfs-law/` project directory currently looks as follows:
>
> ```text
> ├── bin
> │   ├── book_summary.sh
...
To switch to our new branch we can use the `checkout` command that we first saw in Section \@ref(py-rse-git-cmdline-restore):
...
At this point,
`master` and `fit` are two names for the same repository state (Figure \@ref(fig:py-rse-git-advanced-repo-state)),
...
The reason our data points form an approximate straight line on a log-log plot
relates back to the defintion of Zipf's Law,
which says,
...
In other words,
word frequency (\(f\)) is proportional to the inverse rank (\(r\)), \[
...
This involves defining a so-called negative log likelihood function,
whose variables include the word count data and a \(\beta\) parameter.
As we'll see below,
this \(\beta\) parameter is related to the \(\alpha\) parameter in our definition of Zipf's Law
as well as the \(b\) parameter in the general form of a power law.
This means we can use it to verify Zipf's Law *and* plot our fitted curve. 
Expressed as a python function, the likelihood function reads as follows:
...
where the maximum word frequency rank corresponds to \(a\)
in the general form of a power law and \(-\beta + 1\)
corresponds to \(b\).
...
it just isn't included in this branch (Figure \@ref(fig:py-rse-git-advanced-not-lost)).
...
```{r py-rse-git-advanced-not-lost, echo=FALSE, fig.cap="Work Is Not Lost"}
knitr::include_graphics("figures/FIXME.png")
```

We can also look inside `plotcounts.py` and see our changes.
If we make another change and commit it,
that change will also go into the `fit` branch (Figure \@ref(fig:py-rse-git-advanced-change-into-branch)).
For instance,
...
```{r py-rse-git-advanced-change-into-branch, echo=FALSE, fig.cap="Change Goes Into Branch"}
knitr::include_graphics("figures/FIXME.png")
```
...
Merging doesn't change the source branch `fit` (Figure \@ref(fig:py-rse-git-advanced-merging-doesnt-change)).
...
```{r py-rse-git-advanced-merging-doesnt-change, echo=FALSE, fig.cap="Merging Doesn't Change Things"}
knitr::include_graphics("figures/FIXME.png")
```
...
with the `b` changes on top of the earlier `violin` changes:
...
Amira is hosting on GitHub at `https://github.com/amira/zipfs-law`.
...
she sees that it contains all of the changes Amira made.

A copy of a repository is called a [clone][git-clone].
In order to start working on the project,
Sami needs to create a clone of *her* repository (not Amira's) on her own computer.
We will modify Sami's prompt to include her desktop user ID (`sami`)
and her working directory (initially `~`)
to make it easier to follow what's happening:

```shell
sami:~ $ git clone https://github.com/sami/zipfs-law.git
```

FIXME
```text
Cloning into 'zipfs-law'...
...
```shell
jj:~/zipfs-law $ git remote -v
```

```text
origin  https://github.com/sami/zipfs-law.git (fetch)
origin  https://github.com/sami/zipfs-law.git (push)
```

Amira's original repository,
Sami's fork on GitHub,
and Sami's new clone on her desktop
are now arranged as shown in Figure \@ref(fig:py-rse-git-advanced-forked-repos).
Sami can pull changes from her fork and push work back there,
but she needs to do one more thing before she can get changes from Amira's repository:

```shell
sami:~/zipfs-law $ git remote add upstream https://github.com/amira/zipfs-law.git
sami:~/zipfs-law $ git remote -v
```

```text
origin      https://github.com/sami/zipfs-law.git (fetch)
origin      https://github.com/sami/zipfs-law.git (push)
upstream    https://github.com/amira/zipfs-law.git (fetch)
upstream    https://github.com/amira/zipfs-law.git (push)
```

```{r py-rse-git-advanced-forked-repos, echo=FALSE, fig.cap="Forked Repositories"}
knitr::include_graphics("figures/FIXME.png")
```

Sami has called her new remote `upstream` because it points at the repository hers are derived from.
She could use any name,
but `upstream` is a nearly universal convention.

With this remote in place,
Sami is finally set up (Figure \@ref(fig:py-rse-git-advanced-finished-forking)).
Suppose,
for example,
...
```{r py-rse-git-advanced-finished-forking, echo=FALSE, fig.cap="Finished Forking"}
knitr::include_graphics("figures/FIXME.png")
```
...
```shell
amira:~/zipfs-law $ nano README.md
amira:~/zipfs-law $ cat README.md
```
...
```shell
amira:~/zipfs-law $ git commit -a -m "Adding Sami as a contributor"
```
...
```shell
amira:~/zipfs-law $ git push origin master
```
...
```{r py-rse-git-advanced-pull-request, echo=FALSE, fig.cap="Pull Request"}
knitr::include_graphics("figures/FIXME.png")
```

The situation is now as shown in (Figure \@ref(fig:py-rse-git-advanced-pull-request)a),
with Amira's changes on her desktop and in her GitHub repository
but not in either of Sami's repositories.
Since Sami has created a remote that points at Amira's GitHub repository,
though,
she can easily pull those changes to her desktop (Figure \@ref(fig:py-rse-git-advanced-pull-request)b):

```shell
sami:~/zipfs-law $ git pull upstream master
```
...
the difference displayed is always up to date (Figure \@ref(fig:py-rse-git-advanced-how-pr-works)).

```{r py-rse-git-advanced-how-pr-works, echo=FALSE, fig.cap="How Pull Requests Work"}
knitr::include_graphics("figures/FIXME.png")
```
...
(Figure \@ref(fig:py-rse-git-advanced-comment-out-of-date)).
It is common for complex changes to go through several rounds of review and revision
before being merged.

```{r py-rse-git-advanced-comment-out-of-date, echo=FALSE, fig.cap="Comment Out of Date"}
knitr::include_graphics("figures/FIXME.png")
```

To see this in action,
suppose that Sami wants to add her email address to `README.md` in the plotting project.
She creates a new branch and switches to it:

```shell
sami:~/zipfs-law $ git checkout -b adding-email
```
...
```shell
sami:~/zipfs-law $ nano README.md
sami:~/zipfs-law $ git commit -a -m "Adding my email address"
```
...
```shell
sami:~/zipfs-law $ git diff -r HEAD~1
```
...
```shell
sami:~/zipfs-law $ git push origin adding-email
```
...
remote:      https://github.com/sami/zipfs-law/pull/new/adding-email
remote:
To https://github.com/sami/zipfs-law.git
 * [new branch]      adding-email -> adding-email
```

When Sami goes to her GitHub repository in the browser,
...
When Sami clicks on the link in hers,
it takes her to the PR and shows her Sami's comment.
...
```shell
amira:~/zipfs-law $ git pull origin master
```
...
though,
if she also uses `git pull` from her `upstream` repository (i.e., Amira's repository)
so that she's sure to get any other changes that Amira may have merged:

```shell
sami:~/zipfs-law $ git checkout master
```
...
## How can I label a particular version of my project? {#py-rse-git-advanced-tag}
...
### Add new country file and make additional PR {#py-rse-git-advanced-ex-additional-pr}
