...
## How can I test functions manually? {#py-rse-correct-manual}
...
In Python we could use `assert` to achieve the same effect:
...
## What features do we want a testing framework to have? {#py-rse-correct-features}
...
While more information is useful from the unit tests, we don't want an overload of detail.
We only need to see information on what and why a test failed.

Another other requirement for a good set of tests is [isolation][test-isolation].
...
## How do I create and run unit tests in Python? {#py-rse-correct-create-python}
...
It then runs the tests inside the files and summarizes their results.
If you only want to run tests in a particular file,
use the command `pytest path/to/test_file.py`.

Our tests can be saved anywhere in the project directory,
but most projects put them in a subdirectory called `tests`.
...
Tests all have a similar structure. Here's an example.
...
The function being tested generally is not in the file of the tests. 
Test files would normally import those functions, 
but here we have put `add` for convenience and for demonstration.
We don't use comments to explain what each test is doing,
instead we give each test a descriptive name.
That's because `pytest` will print the failed test fails.
...
and the function's result set as a variable so multiple checks can be used.
...
Let's fix the name and re-run the tests:
...
## How should I set up fixtures in my unit tests? {#py-rse-correct-fixtures}
...
TODO: Include example Python code
...
The simplest way to do this is the Python `assert` statement,
which [raises][raise] an [exception][exception] if a condition is not true.
...
but each function only runs as far as the first failed check (Figure \@ref(fig:py-rse-correct-failed-test)).
...
## How can I tell whether my code has been tested or not? {#py-rse-correct-coverage}
...
Even short programs,
loops or conditionals can make it difficult for us to figure out
...
the tool shows us the lines that were evaluated
and report summary statistics like the number or percentage of lines that weren't used.
...
## How much testing is enough? {#py-rse-correct-enough}
...
## Why should I be cautious when using floating-point numbers? {#py-rse-correct-float}

TODO: How necessary is the below paragraph? Doesn't feel it fits with the course content (its too detailed).
TODO: This section could be reduced substantially and merged in with another section. Its too much of a tangent.
...
## How should I write tests that involved floating-point values? {#py-rse-correct-numeric}

TODO: Merge this section with previous section and reduce content of both.
...
## How can I test a program that uses random numbers? {#py-rse-correct-random}
...
### Checking simple calculations {#py-rse-correct-ex-simple-calc}

Create a new test by ...
...
### Testing `count_words` {#py-rse-correct-ex-count-words}
...
### Checking for errors {#py-rse-correct-ex-check-errors}
