...
and a constant \(c\) of proportionality (Section \@ref(py-rse-git-advanced-zipf-math)).\[
r = cf^{\frac{-1}{\alpha}}
\]
...
## Testing error handling {#py-rse-correct-failure}
...
In Section \@ref(py-rse-git-advanced-zipf-verify)
...
they clean it up (Section \@ref(py-rse-style-refactor)) and then move on to the next task.
...
## Continuous integration {#py-rse-correct-ci}
...
(which is described in Section \@ref(py-rse-git-advanced-fork)),
...
on the left-hand side of your Travis CI homepage (\@ref(fig:py-rse-correct-add-repo)).

```{r py-rse-correct-add-repo, echo=FALSE, fig.cap="Click to add a new GitHub repository to Travis CI"}
...
```{r py-rse-correct-list-repos, echo=FALSE, fig.cap="Find your Zipf's Law repository and switch it on"}
...
and is written in the [YAML][yaml] format we met earlier (Chapter \@ref(py-rse-configuration)).
...
shown in red.
In this case, we can see that the build failed (Figure \@ref(fig:py-rse-correct-build-fail)).

```{r py-rse-correct-build-fail, echo=FALSE, fig.cap="Travis Build Overview"}
knitr::include_graphics("figures/rse-correct/travis-build-fail.png")
```
...
and our tests pass (Figure \@ref(fig:py-rse-correct-build-pass)).

```{r py-rse-correct-build-pass, echo=FALSE, fig.cap="Travis Build Overview"}
