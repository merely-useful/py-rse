...
For example,
this code halts as soon as the loop encounters a
weather station rainfall observation that isn't positive:

```python
rainfall_observations = [1.5, 2.3, 0.7, -0.2, 4.4]
total = 0.0
for obs in rainfall_observations:
    assert obs >= 0.0, 'Rainfall values must be non-negative'
    total += obs
...
<ipython-input-19-33d87ea29ae4> in <module>()
      2 total = 0.0
      3 for obs in rainfall_obs:
----> 4     assert obs > 0.0, 'Rainfall values must be non-negative'
      5     total += obs
      6 print('total rainfall is:', total)

AssertionError: Rainfall values must be non-negative
```
...
A user could easily overlook the docstring at the top of the script,
which states that it combines multiple CSV files containing word counts
to create a single cumulative count.
If they pass a set of plain text files instead of CSV files,
the error message `collate.py` produces isn't particularly helpful:
...
An assertion in the `main` function of `collate.py`
makes the requirement clear in the code,
and is also a convenient place to write a better error message:
...
The poem "Risk" by Anaïs Nin is a good choice:
...
## Testing Frameworks {#correct-framework}
...
## Testing Floating-Point Values {#correct-numeric}
...
on the left-hand side of the Travis CI homepage (\@ref(fig:correct-add-repo)).
...
that sites notifies Travis CI that the repository has changed.
...
In this case,
we can see that the build failed (Figure \@ref(fig:correct-build-fail)).
...
(Figure \@ref(fig:correct-build-pass)).
...
## When to Write Tests {#correct-tdd}
...
### Explaining assertions {#correct-ex-explain-assertions}
...
### Test assertions {#correct-ex-test-assert}
