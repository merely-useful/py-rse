...
We'll start with our Zipf's Law project files as they were structured at the end of the previous chapter:
...
def test_num_sign():
    assert num_sign(0.1) == 1
    assert num_sign(0) == 0
    assert num_sign(math.inf) == -1
```
...
Do exercise \@ref(py-rse-correct-ex-numsign) on fixing `num_sign()` so the tests pass.

## What does a testing framework have? {#py-rse-correct-features}

The individual tests in `test_num_sign()` are called [unit tests][unit-test]
...
In this case, we don't know if `num_sign()` handles `-Inf` correctly or not because
testing stops when `num_sign(0)` gives the wrong answer.
...
FIXME: summarize correctness chapter.
