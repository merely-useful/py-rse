# Code Style, Review, and Refactoring {#py-rse-style}
...
This lesson therefore presents guidelines for good Python programming style
and introduces some language features that can make programs more flexible and more readable.

## What is good style for Python programs? {#py-rse-style-pep8}
...
:   Code review isn't just (or even primarily) about style:
    ...
    Are errors being caught and handled (Section \@ref(py-rse-errors-logging))?
...
but the legacy of those times lives on.
...
for logging purposes (Section \@ref(py-rse-errors-logging)).
If you are sure you don't need the code,
take it out completely:
you can always get it back from version control
(Section \@ref(py-rse-git-cmdline-ex-recover)).
...
## In what order should functions be defined? {#py-rse-style-order}

The order of items in each file should be:

-   The [shebang][shebang] line (because it has to come first to work).
-   The file's documentation string (Section \@ref(py-rse-package-py-document)).
...
-   If the file can be run as a program,
    ...
    Section \@ref(py-rse-package-py-import).
...
We prefer the former approach,
i.e.,
putting high-level functions at the top of the file.
The reason is that when people encounter code for the first time,
they usually scan it from top to bottom.
The first function they read should therefore help them understand
what the rest of the file contains and where to look for it.
Three common patterns that people use for this are:

1.  Figure out what the user has asked it to do (Chapter \@ref(py-rse-configuration)).
...
since it signals even more clearly which functions are specific to this program
and which are likely to be reused elsewhere.

## How can I check that code follows style guidelines? {#py-rse-style-check}
...
## How should I review code? {#py-rse-style-review}

Now that our little program gets a clean bill of health from `pycodestyle`,
it's worth having a human being look it over.
Here are things that should still be changed:

-   The commented-out `print` statements should either be removed
    or turned into logging statements (Section \@ref(py-rse-errors-logging)).
...
It is also more difficult to do in specialized scientific fields:
...
## How can I do code reviews on GitHub? {#py-rse-style-how}

FIXME: how to review on GitHub.

## How can I specify default values for functions' parameters? {#py-rse-style-defaults}
...
## How can I write functions to handle a variable number of arguments? {#py-rse-style-varargs}
...
## How can I pass an unknown number of parameters? {#py-rse-style-starargs}
...
## How can I get values out of structured data more easily? {#py-rse-style-destructure}
...
## What should I look for when reviewing code? {#py-rse-style-refactor}
...
Issues like this are why experienced developers write unit tests (Chapter \@ref(py-rse-correct))
...
## Exercises {#py-rse-style-exercises}

FIXME: create exercises for review.

## Key Points {#py-rse-style-keypoints}
