...
FIXME add connection with previous chapters, specifically version control, teams, etc
...
-   A dataset that is being used by several research projects.
    ...
    the tidied data,
    the extra files needed to make the dataset an R package (Chapter \@ref(py-rse-package-r)),
...
-   `LICENSE` is the project's license (discussed in Section \@ref(py-rse-teams-license)).

-   `CONDUCT` is its code of conduct (also discussed in Section \@ref(py-rse-teams-coc)).
...
but two of these (`LICENSE` and `CONDUCT`) are usually chosen rather than written (Chapter \@ref(py-rse-teams-license))
...
Once someone knows that a project is using the MIT License (Section \@ref(py-rse-teams-software-license))
and the Contributor Covenant (Section \@ref(py-rse-teams-conduct)),
...
-   The `DESCRIPTION` file in the root directory contains
    the information needed to make the project an R package (Chapter \@ref(py-rse-package-r)).
...
or as a set of tasks in its issue tracking system (Section \@ref(py-rse-teams-issues)).
