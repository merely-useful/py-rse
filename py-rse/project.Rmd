# Project Structure {#py-rse-project}

```{r py-rse-project-setup, include=FALSE}
...
We'll be referencing the current files in our Zipf's Law project directory:
...
We'll discuss the rationale for why our project is set up this way,
as well as other approaches that may assist in your development of other projects.
...
## What is a project? {#py-rse-project-thinking}

In our example for this book,
our project includes the code,
data, and documentation associated with our Zipf's Law project,
contained within `zipfs_law/`.
We've modeled an appropriate organizational structure as the analysis developed,
so we don't need to reorganize much now.

In cases where you haven't been told what is included in the project,
...
## What files should every project contain? {#py-rse-project-boilerplate}
...
-   `LICENSE` is the project's license (Section \@ref(py-rse-teams-license)).

-   `CONDUCT` is its code of conduct (Section \@ref(py-rse-teams-coc)).
...
-   `CONTRIBUTING`
    ...
    what tags to put on issues (Section \@ref(py-rse-teams-planning)),
...
-   `GOVERNANCE`
    explains how the project is run (Section \@ref(py-rse-teams-martha)).
    ...
    FIXME connect with material in teams

Including four core files may seem like a lot,
but two of these (`LICENSE` and `CONDUCT`) are usually chosen rather than written (Section \@ref(py-rse-teams-license))
...
Once someone knows that a project is using the MIT License (Section \@ref(py-rse-teams-license))
and the Contributor Covenant (Section \@ref(py-rse-teams-coc)),
they don't need to read the files to know what's there.

Of the four standard files generally present in most projects,
we have three already present in our Zipf's Law project.
`README.md` was created as a part of Chapter \@ref(py-rse-git-cmdline)
`CONDUCT.md` and `LICENSE.md` were both created in Chapter \@ref(py-rse-teams).
At this point in our project development,
it doesn't make sense to create separate files for the other standard files.
However, you may choose to include subheadings in the main directory's `README.md`
with information about contributing to the project
Eventually,
you may also include governance processes,
citation details,
and contributors.

## How should I structure the rest of my project? {#py-rse-project-organize}
...
-   The `src/` directory contains a Makefile to re-build the `regulate` program (Chapter \@ref(py-rse-automate)).
...
The key differences between this structure and previous structures discussed are:

-   The `DESCRIPTION` file in the root directory contains
    the information needed to make the project an R package.
    FIXME: this chapter is supposed to be about Python, not R.
...
FIXME (make this better) Going in the other direction,
...
With this understanding of different approaches to file organization in projects,
the structure and filenames for our Zipfs Law project should be clear.
Add information to the main directory's `README.md` describing the contents of `bin/`,
`data/`, and `results/`.

> **Reorganizing Files**
> Think about your project organization sooner rather than later.
> If you decide on a different directory structure later,
> be aware of how locations of data and scripts are referenced in paths in other scripts.

## How should I manage a mix of compiled programs and scripts? {#py-rse-project-scripts}
...
and saves those instructions in files (Figure \@ref(fig:py-rse-project-languages)a).
...
but puts those instructions in memory and executes them immediately (Figure \@ref(fig:py-rse-project-languages)b).

```{r py-rse-project-languages, echo=FALSE, fig.cap="Language Types"}
...
In practice,
software developers handle all of this in different ways:
...
In our Zipfs Law project,
we don't have source files,
so both shell (`*.sh`) and Python (`*.py`)
files are included in `src/`.

## How should I document the software in a project? {#py-rse-project-software-docs}
...
1.  Most projects generate the documentation for their software directly from the source code
    (Section \@ref(py-rse-package-py-document)).
...
## What should I document? {#py-rse-project-infer}
...
as discussed in Chapter \@ref(py-rse-correct),
these should have long names that describe what they're checking
so that failure reports are easy to scan.

We've included docstring for each function created so far.
Review this documentation and determine whether the information is sufficient for continued development of the project.

## How can I create a useful FAQ? {#py-rse-project-faq}
...
:   Section \@ref(py-rse-teams-bugs) explains the value of a [reproducible example][reprex] (reprex),
...
## How does documentation for data differ from documentation for code? {#py-rse-project-data-vs-code}
...
## How should I document the data in a project? {#py-rse-project-data-docs}
...
Multiple styles of material will help different people best (Section \@ref(py-rse-project-faq)),
...
-   **Give every part of the data a unique identifier.**
    ...
    To achieve this,
    the dataset itself should have a [DOI][doi] (Section \@ref(py-rse-publish-identifiers)),
    and within the dataset,
    every file should have a meaningful name (Section \@ref(py-rse-project-data-vs-code)).
    Going further,
    every record should have a unique [key][key] within the dataset (Section \@ref(py-rse-publish-data)):
...
## How can I get started documenting my data? {#py-rse-project-data-start}
...
or as a set of tasks in its issue tracking system (Section \@ref(py-rse-teams-planning)).
...
## What data documentation should I create first? {#py-rse-project-data-first}

The first thing to produce when docmenting data is an inventory of what data is available.
This inventory collects important information for the team during the active collection and analysis process.  
...
-   Where are any physical representations of this data stored?
    This is relevant for anything with surveys, samples, specimens, interview sheets, etc.
...
Our Zipf's Law data came from an external source,
so a `README` was included in `data/`.
Take a look at this information and evaluate whether the content is sufficient for the scope of the project.

## How should I manage data that can't be stored in version control? {#py-rse-project-external}
...
FIXME: move this up Another option is to have one file per dataset,
...
(Section \@ref(py-rse-publish-data)).

## Summary {#py-rse-project-summary}

FIXME: summarize project structure chapter

## Exercises {#py-rse-project-exercises}

The exercises are modified from @Wick2016 and explore the dataset from @Meil2015.
They try to understand the relationship between the study and the included files.
...
### Understand a project {#py-rse-project-ex-understand-project}
...
### Determining the audience for documentation {#py-rse-project-ex-audience}

The first step to creating documentation is to think about the audience over time.
...
### Creating documentation template {#py-rse-project-ex-create-doc-template}
...
### Making permanent links {#py-rse-project-ex-permanent-links}
...
## Key Points {#py-rse-project-keypoints}
