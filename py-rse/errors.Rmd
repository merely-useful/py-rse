...
The code below shows a very simple example
that uses exceptions to report attempts to divide by zero:
...
Internal errors should be prevented by doing unit testing (Chapter \@ref(correct)),
...
We can aso use that function to set the logging level;
...
and explain why we use `str.format` rather than f-strings for formatting error messages.
