...
This least to the rule "throw low, catch high":
...
But what happens if we are trying to read an file that exists,
...
It would be useful if our programmed check that the input files were CSV-files,
and if not,
raised an error with a useful explanation to what went wrong.
...
```shell
$ python bin/collate.py Makefile                                                                          1 ↵
```
...
```python
LOG_LEVEL = 2  # Or some other number
...
by default,
the source is called "root" for reasons that will become clear later.
Thus,
