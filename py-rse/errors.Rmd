...
Section \@ref(py-rse-correct-failure) described some ways to write unit tests
...
1.  Modify the script `collate.py` so that
    it prints the names of the files its opening to standard error
    if the logging level is set to `INFO` or higher.
2.  Modify the way command-line arguments are handled
    so that users can set the logging level from the command line.
