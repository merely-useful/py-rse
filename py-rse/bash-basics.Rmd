...
the text files for several ebooks from Project Gutenberg:
...
```shell
$ wc -l *.txt | sort -n | head -n 3 | tail -n 1
```
...
```shell
$ wc -l *.txt | sort -n | head -n 3 | tail -n 1 > results.txt
```
...
```text
  ...
  553  wc -l *.txt | sort -n | head -n 3 | tail -n 1
```
...
Suppose we want to create a backup copy of each book whose name starts with m.
...
```shell
$ cp data/moby_dick.txt backup/moby_dick.txt
```

but can't do all the files at once:

```shell
$ cp data/m*.txt backup/m*.bak
```
...
`backup/*.bak` doesn't match anything—those files don't yet exist—so
what we are actually asking `cp` to do is:

```shell
$ cp moby_dick.txt mysterious_affair_at_styles.txt
```
...
```text
moby_dick.txt.bak  mysterious_affair_at_styles.txt.bak
```
