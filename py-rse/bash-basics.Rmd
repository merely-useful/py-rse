...
knitr::include_graphics("figures/py-rse-bash-basics/the_shell.png")
```
...
remembering that what you see might deviate because of your operating system or other configurations.
...
knitr::include_graphics("figures/py-rse-bash-basics/sample_filesystem.png")
```
...
Desktop      Downloads    Movies       Pictures     zipfs_law
```
...
> ```
>
> Consider this a convenience allowed by some (but not all) computers,
> and try to avoid these shortcuts when writing code you intend other people to use.
...
If we want to see what's in `zipfs_law`,
...
Notice that `zipfs_law` doesn't have a leading slash before its name.
...
but if we are going to do a lot of work with the data in the `zipfs_law` directory,
the easiest thing would be to change our current working directory
so that we don't have to type `zipfs_law` at all.
...
> This is because some commands take long-name options,
> and it's very easy to mistake `-no` (meaning `-n -o`)
> with `--no` (meaning "answer 'no' to all questions").

The special names `.` and `..` don't belong to `cd`:
they are interpreted the same way by every program.
For example,
if we are in `/Users/amira/zipfs_law`,
...
because there are fewer interactions and exceptions to remember.
...
(e.g., from `/Users/amira/zipfs_law` to `/Users`),
while `cd ../Movies` will move us up from `zipfs_law` and back down into `Movies`.
...
To find out,
let's go back to our `zipfs_law` directory:
...
knitr::include_graphics("figures/py-rse-bash-basics/nano_editor.png")
```
...
Let's tidy up by removing the `prior-work.txt` file we created in our `zipfs_law` directory.
...
The `data` directory in our zipfs_law project contains
...
Let's go into the `zipfs_law/data` directory
...
If we only want to view the shortest book:

```shell
$ wc -l *.txt | sort -n | head -n 1
```

We could also redirect the output to a file by adding `> shortest.txt`
...
knitr::include_graphics("figures/standard_io.png")
```
...
`sort` will then sort and print whatever we typed (Figure \@ref(fig:py-rse-bash-basics-pipe)a):
...
(Figure \@ref(fig:py-rse-bash-basics-pipe)b).
...
```{r py-rse-bash-basics-pipe, echo=FALSE, fig.cap="Piping commands"}
...
In this case,
a better variable name would be `bookname`.
...
> you may have received the following warning:
>
> ```text
> mkdir: backup: File exists
> ```
>
> This warning is an FYI,
> that lets you know the action couldn't be completed,
...
But what if we want the backup files to have the [extension][filename-extension] `.bak` appended?
...
## Summary {#py-rse-bash-basics-summary}

FIXME: summarize Bash basics chapter
...
you can start with a fresh copy of the data files downloaded from [here](FIXME).
...
knitr::include_graphics("figures/exercise-filesystem.png")
```
