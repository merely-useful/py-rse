# The Basics of the Unix Shell {#py-rse-bash-basics}

```{r py-rse-bash-setup, include=FALSE}
...
and in essence is a program that runs other programs on our behalf (Figure \@ref(fig:py-rse-bash-basics-repl)).
...
```{r py-rse-bash-basics-repl, echo=FALSE, fig.cap="The Shell"}
...
## How can I explore my files and directories using the shell? {#py-rse-bash-basics-explore}
...
Amira is one of the learners described in Section \@ref(py-rse-intro-personas);
...
On Amira's computer, it looks like Figure \@ref(fig:py-rse-bash-basics-filesystem).

```{r py-rse-bash-basics-filesystem, echo=FALSE, fig.cap="Sample Filesystem"}
knitr::include_graphics("figures/FIXME.png")
```
...
Desktop      Downloads    Movies       Pictures     zipfs-law
```
...
## How can I move around in the shell? {#py-rse-bash-basics-navigate}
...
If we want to see what's in `zipfs-law`,
...
```text
bin/  data/  Makefile  makefiles/
```

Notice that `zipfs-law` doesn't have a leading slash before its name.
...
but if we are going to do a lot of work with the data in the zipfs-law directory,
the easiest thing would be to change our current working directory
so that we don't have to type `zipfs-law` at all.
...
For example,
if we are in `/Users/amira/zipfs-law`,
...
(e.g., from `/Users/amira/zipfs-law` to `/Users`),
while `cd ../Movies` will move us up from `zipfs-law` and back down into `Movies`.
...
## How can I create new files and directories? {#py-rse-bash-basics-filedir}

We now know how to explore files and directories,
but how do we create them?
To find out,
let's go back to our `zipfs-law` directory:
...
```text
analysis/  bin/  data/  Makefile  makefiles/
...
(Figure \@ref(fig:py-rse-bash-basics-nano)):
...
```{r py-rse-bash-basics-nano, echo=FALSE, fig.cap="The Nano Editor"}
knitr::include_graphics("figures/FIXME.png")
```
...
## How can move files and directories around? {#py-rse-bash-basics-move}
...
```text
analysis/  bin/  data/  Makefile  makefiles/  prior-work.txt
```
...
## How can I copy files and directories? {#py-rse-bash-basics-copy}
...
## How can I delete files and directories? {#py-rse-bash-basics-rm}

Let's tidy up by removing the `prior-work.txt` file we created in our `zipfs-law` directory.
...
## How can I run commands on lots of files at once? {#py-rse-bash-basicswildcard}

The `data` directory in our zipfs-law project contains
the text files for several ebooks from [Project Gutenberg][project-gutenberg]:
...
```text
anne_of_green_gables.txt        moby_dick.txt
common_sense.txt                mysterious_affair_at_styles.txt
count_of_monte_cristo.txt       pride_and_prejudice.txt
dracula.txt                     README.md
emma.txt                        sense_and_sensibility.txt
ethan_frome.txt                 sherlock_holmes.txt
frankenstein.txt                time_machine.txt
jane_eyre.txt                   treasure_island.txt
life_of_frederick_douglass.txt
```
...
> **What's in a Word?**
>
> note: `wc` only considers spaces to be word breaks, not commas.
...
so `data/*.csv` matches all of the files in the `data` directory:
...
```text
data/anne_of_green_gables.txt   data/life_of_frederick_douglass.txt
data/common_sense.txt           data/moby_dick.txt
data/count_of_monte_cristo.txt  data/mysterious_affair_at_styles.txt
data/dracula.txt                data/pride_and_prejudice.txt
data/emma.txt                   data/sense_and_sensibility.txt
data/ethan_frome.txt            data/sherlock_holmes.txt
data/frankenstein.txt           data/time_machine.txt
data/jane_eyre.txt              data/treasure_island.txt
```

while `data/c*.txt` only matches the four whose names begin with a 'b':
...
```text
data/common_sense.txt  data/count_of_monte_cristo.txt
```
...
count the number of words in the books with names that start with t:
...
```text
  3582  35527 204510 data/time_machine.txt
  7886  71619 396614 data/treasure_island.txt
 11468 107146 601124 total
```

or the number of words in Pride and Prejudice:
...
```text
13707 124974 725074 data/pride_and_prejudice.txt
```
...
## How can I find out what commands there are and how to use them? {#py-rse-bash-basics-help}
...
## How can I combine commands? {#py-rse-bash-basics-pipe}

Now that we know a few basic commands,
we can introduce one of the shell's most powerful features:
the ease with which it lets us combine existing programs in new ways.
Let's go into the `zipfs-law/data` directory
...
```text
   11179 anne_of_green_gables.txt
    2660 common_sense.txt
   61162 count_of_monte_cristo.txt
   15975 dracula.txt
   16632 emma.txt
    4202 ethan_frome.txt
    7832 frankenstein.txt
   21054 jane_eyre.txt
    4102 life_of_frederick_douglass.txt
   22331 moby_dick.txt
    8933 mysterious_affair_at_styles.txt
   13707 pride_and_prejudice.txt
   13028 sense_and_sensibility.txt
   13053 sherlock_holmes.txt
    3582 time_machine.txt
    7886 treasure_island.txt
  227318 total
```

Which of these books is shortest?
It's possible to see when there are only 16 files,
...
but keeping track of those files and cleaning them up when they're no longer needed is a burden. Let's delete them for now:
...
For example,
we can use the command `head` to get just the first three lines of sorted data, the three shortest books:
...
If we only want to save the shortest book:

```shell
$ wc -l *.txt | sort -n | head -n 1 > shortest.txt
```
...
## How do pipes work (and why do I need to know)? {#py-rse-bash-basics-stdio}
...
so it reads what we type and displays its output for us to see (Figure \@ref(fig:py-rse-bash-basics-stdio)a).
When we tell the shell to run a program
it creates a new process
and temporarily reconnects the keyboard and stream
to that process's standard input and output (Figure \@ref(fig:py-rse-bash-basics-stdio)b).

```{r py-rse-bash-basics-stdio, echo=FALSE, fig.cap="Standard I/O"}
knitr::include_graphics("figures/FIXME.png")
```

Redirection with `>` tells the shell to connect the program's standard output to a file
instead of to the screen (Figure \@ref(fig:py-rse-bash-basics-stdio)c).
...
(Figure \@ref(fig:py-rse-bash-basics-misconnect)).
...
```{r py-rse-bash-basics-misconnect, echo=FALSE, fig.cap="Misconnection"}
knitr::include_graphics("figures/FIXME.png")
```
...
## How can I do the same operations on many files? {#py-rse-bash-basics-loops}
...
Let's suppose that we want to take a section out of each book whose name starts with an s in the `data` directory—
more specifically,
that we want to get the first 19 rows of each file
*without* the header rows that are the first few lines.
If we only cared about one file,
we could use a simple pipeline to take the first 20 lines
and then take the last 11 of those:

```shell
$ head -n 19 sense_and_sensibility.txt | tail -n 11
```
...
we only get 11 lines of output,
not the 22 we expect:

```shell
$  head -n 19 s*.txt | tail -n 11
```
...
3.  The loop runs once for each item in the list.
    ...
    so in this case,
    `filename` will be `sense_and_sensibility.txt` the first time around
...
Let's use a wildcard expression to select both books and add another stage to the pipeline:

```shell
$ for filename in s*.txt
> do
>   head -n 19 $filename | tail -n 11
> done
```
...
The first printed row for each file, which contains the title, shows that
we are in fact getting exactly the rows intended from each of the two files.
...
## What do variable names mean? {#py-rse-bash-basics-meaningless}
...
```shell
$ for bookname in s*.txt
> do
>   head -n 19 $bookname | tail -n 11
> done
```
...
## How can I re-do things I have done recently? {#py-rse-bash-basics-history}
...
```shell
$ for filename in s*.txt ; do  head -n 19 $filename | tail -n 11; done
```
...
```shell
$ for filename in s*.txt; do echo $filename; head -n 19 $filename | tail -n 11; done
```

## How can I create new filenames automatically? {#py-rse-bash-basics-autoname}

Suppose we want to create a backup copy of each book whose name has the preposition 'of' in it.
...
$ cd ~/zipfs-law
$ mkdir backup
$ cp data/*of*.txt backup
```

But what if we want the backup files to have the [extension][filename-extension] `.bak` instead of `.txt`?
`cp` can do this for a single file:

```shell
$ cp data/anne_of_green_gables.txt backup/anne_of_green_gables.bak
```

but can't do all the files at once:

```shell
$ cp data/*of*.txt backup/*of*.bak
```
...
`backup/*of*.bak` doesn't match anything—those files don't yet exist—so
what we are actually asking `cp` to do is:

```shell
$ cp data/anne_of_green_gables.txt   data/life_of_frederick_douglass.txt
data/count_of_monte_cristo.txt
```
...
```text
anne_of_green_gables.bak   life_of_frederick_douglass.bak
count_of_monte_cristo.bak
```

## Exercises {#py-rse-bash-basics-exercises}

### Exploring more `ls` flags {#py-rse-bash-basics-ex-more-ls}
...
### Listing recursively and by time {#py-rse-bash-basics-ex-ls-rt}
...
### Absolute and relative paths {#py-rse-bash-basics-ex-paths}
...
### Relative path resolution {#py-rse-bash-basics-ex-resolve-rel-path}

Using the filesystem shown in Figure \@ref(fig:py-rse-bash-basics-ex-rel-path),
...
```{r py-rse-bash-basics-ex-rel-path, echo=FALSE, fig.cap="Exercise Filesystem"}
knitr::include_graphics("figures/FIXME.png")
```

### `ls` reading comprehension {#py-rse-bash-basics-ex-reading-ls}

Using the filesystem shown in Figure \@ref(fig:py-rse-bash-basics-ex-reading-ls),
...
```{r py-rse-bash-basics-ex-reading-ls, echo=FALSE, fig.cap="Filesystem for Exercise"}
knitr::include_graphics("figures/FIXME.png")
```
...
### Creating files a different way {#py-rse-bash-basics-ex-touch}
...
### Moving to the current folder {#py-rse-bash-basics-ex-move-dot}
...
### Renaming files {#py-rse-bash-basics-ex-renaming-files}
...
### Moving and copying {#py-rse-bash-basics-ex-last-ls}
...
### Using `rm` safely {#py-rse-bash-basics-ex-safe-rm}

What happens when we execute `rm -i thesis_backup/quotations.txt`?
Why would we want this protection when using `rm`?

### Copy with multiple filenames {#py-rse-bash-basics-ex-copy-multi}
...
### List filenames matching a pattern {#py-rse-bash-basics-ex-ls-match}
...
### More on wildcards {#py-rse-bash-basics-ex-more-wildcards}
...
### Organizing directories and files {#py-rse-bash-basics-ex-organizing}
...
### Reproduce a directory structure {#py-rse-bash-basics-ex-reproduce-structure}
...
### What does `sort -n` do? {#py-rse-bash-basics-ex-sort-n}
...
### What does `>>` mean? {#py-rse-bash-basics-ex-redirect-append}
...
### Appending data {#py-rse-bash-basics-ex-append-data}
...
### Piping commands {#py-rse-bash-basics-ex-piping}
...
### Why does `uniq` only remove adjacent duplicates? {#py-rse-bash-basics-ex-uniq-adjacent}
...
### Pipe reading comprehension {#py-rse-bash-basics-ex-reading-pipes}
...
### Pipe construction {#py-rse-bash-basics-ex-pipe-construction}
...
### Which pipe? {#py-rse-bash-basics-ex-which-pipe}
...
### Wildcard expressions {#py-rse-bash-basics-ex-wildcard-expressions}
...
### Removing unneeded files {#py-rse-bash-basics-ex-remove-unneeded}
...
### Doing a dry run {#py-rse-bash-basics-ex-loop-dry-run}
...
### Nested loops {#py-rse-bash-basics-ex-nested-loops}
...
### Variables in loops {#py-rse-bash-basics-ex-loop-variables}
...
### Limiting sets of files {#py-rse-bash-basics-ex-limiting-file-sets}
...
### Saving to a file in a loop {#py-rse-bash-basics-ex-loop-save}
...
### List unique species {#py-rse-bash-basics-ex-list-unique}
...
### Why does `history` record commands before running them? {#py-rse-bash-basics-ex-history-order}
...
## Key Points {#py-rse-bash-basics-keypoints}
