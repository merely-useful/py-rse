...
This then gives immediate feedback to the project developers if the changes cause potential problems, 
...
makes a copy of the project (by cloning),
...
via GitHub's [API][api] in order to build and run the project.
Without this permission,
we would have to enter our password every time Travis CI wanted to run on a repo,
which would hardly count as "automated". 
Even so, we should always check carefully when authorizing a service like this
...
The `language` setting (or key) tells Travis CI which programming language to use
so that it knows which of its standard [virtual machines][virtual-machine] to use
as a starting point for the project.

Next, let's add a line telling Travis which version of Python to use with the `python` setting:
...
Here we've added a setting called `script`
...
And because Travis runs in a "clean" environment (aka, not your own computer),
...
The `install` setting tells Travis CI how to install the software we need for our project.
For Python projects,
...
TODO: Will requirements.txt already be created in the Package chapter?
...
We use the Travis setting `after_success` since the coverage tool is a separate
service and the Travis build should happen first.
We use [`codecov`][codecov] rather than the `coverage` tool described in Section \@ref(py-rse-correct-coverage)
since it is a service like Travis CI while the `coverage` tool is a program.
...
1. Add the `codecov` command to the `after_success` setting in the `.travis.yml`
