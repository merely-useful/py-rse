...
Our Zipf's Law project structure should include the following:
...
Let's consider some hypothetical Python code.
...
We recommend moving the template script outside your project directory,
so you can reference it for later projects as well.
We'll move to `bin/`,
where we previously stored our shell script,
before we start creating more scripts:

```shell
$ cd ~/zipf/bin
```

Before we create our scripts for Zipf's Law,
let's think through the tasks our scripts will need to accomplish.
...
```shell
$ python countwords.py ../data/dracula.txt > ../results/dracula.csv
```

```shell
$ python countwords.py ../data/moby_dick.txt > ../results/moby_dick.csv
```

```shell
$ python countwords.py ../data/jane_eyre.txt > ../results/jane_eyre.csv
```

In the code above,
we created a subdirectory in our project directory to hold our files containing output from data analysis.
We'll talk more in Chapter \@ref(py-rse-project) to explain this organization.
...
from the each of the csv-files created from `countwords.py`.
...
opens the CSV-file,
and runs the `update_counts` function with the file object as one of the parameters.
This function iterates through all the words in each of the CSV-files
and adds up the counts with the `+=` operator.

You will notice that we have not used `type=argparse.FileType('r')` here.
This is to show how the syntax would look when we use filenames with `open` instead,
and because we are not interested in passing standard input directly to the `collate.py` script.
Passing the filename rather than its content
is also useful when you are intending to e.g. move or copy the file.

The `collate.py` script will take an arbitrary number of input files
generated using `countwords.py` and collate the word counts:

```shell
$ python collate.py ../results/dracula.csv ../results/moby_dick.csv ../results/jane_eyre.csv -n 10
```
...
> **Ignoring the `__pycache__` Directory**
>
...
```{r py-rse-py-scripting-repl, echo=FALSE, fig.cap="Word frequency distribution for the book Jane Eyre"}
