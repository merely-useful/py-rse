...
(also sometimes called a [switch][command-switch], a [flag][command-flag], or an [argument][command-argument]).
Options are exactly like parameters to a function in R or Python;
in this case,
...
We can combine the two types of options:
...
but we must put the command's built-in options (like `-F`)
before the names of any files or directories we want to work on
because once the command encounters a filename,
...
> when we run `ls`.
> We can always use the `-a` flag to display them.
...
`cd` without any options always returns us to our home directory.
...
(`ls` doesn't show any trailing slashes because we haven't used `-F`.)
We can use `~` in paths,
so that (for example) `~/Downloads` always refers to our download directory.

Finally,
we can use the shortcut `-` (a single dash) to mean the last directory we were in.
This is faster and more reliable than trying to remember and type the path.
Unlike `~`,
this only works with `cd`:
...
```shell
$ cp quotes.txt analysis/quotations.txt
$ ls quotes.txt analysis/quotations.txt
```

```text
quotes.txt   analysis/quotations.txt
```
...
or try <kbd>Ctrl</kbd>+<kbd>Spacebar</kbd> and <kbd>Spacebar</kbd> to skip up
and down by a full page (<kbd>B</kbd> and <kbd>F</kbd> also works).
