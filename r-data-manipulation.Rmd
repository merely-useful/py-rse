...
Use `arrange()` to order the dog licenses by `AnimalName` in increasing order.  What does this tell you about the way R treats punctation and numbers when dealing with alphabetical order?
...
Read the following code aloud to your neighbour (or cat, dog or rubber duck).  Remember to pronounce `%>%` as "and then".  
```{r, eval = FALSE}
dog_licenses %>% 
  filter(AnimalGender == "F") %>% 
  arrange()
```
What do you think the code does? What question does it answer?
...
This code creates two variables that contain dates for the start and end of the year 2016:
```{r}
start_of_2016 <- as.Date("2016-01-01")
end_of_2016  <- as.Date("2016-12-31")
```

Use `filter()` with these variables to find:

* The dog licenses that were issued before 2016
* The dog licenses that expire during 2016
...
Use arrows to sort, and search boxes to filter. Similar actions in other programs.

WHich dog has 
Remove dogs with expired licenses in 2016? Or keep dogs with licenses issued in 2016.
...
How long are dogs names? What is the longest name?  What is the shortest name?
...
What month has the most registrations?
Which birth month has the most registered dogs?
