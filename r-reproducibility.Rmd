...
1. Go to (link to html page). This is a HTML file that has been created by using
Markdown formatting. 
...
can create a `html_document` that looks exactly like that (linked) file.
4. Knit the 
...
the top of the document, surrounded by `---`. The YAML contains the metadata and
various options for the entire document. For instance, the title and/or author 
text is included here, as well as the options on what the converted output
document should be, such as Word, HTML, or PDF (if you want to create a beautiful
PDF, you need to install the R package [tinytex][tinytex]). While these three
are the most commonly used output formats, there are many more output formats to
choose from (e.g. slides, websites, books). The YAML header looks something like
this:
...
which we will cover later on. In this particular case, the R Markdown file
will convert to a HTML document via the `html_document` setting. There are also
`word_document` and `pdf_document` settings. For now, we'll focus on the
...
**Note**: Code [chunk labels][chunk-label] should be named without `_`, spaces,
or `.` and instead should be one word or be separated by `-`. While an error may
not necessarily occur, there can be some unintended side effects that will cause
you some annoyance without knowing the reason.
...
dog_licenses <- read_csv(here::here("data/..."))
`r ''````
````
...
We call the code chunk `setup` because it is getting everything ready for later 
code.
...
````markdown
`r ''````{r dog-license-plot}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
`r ''````
````

```{r dog-license-plot, echo=FALSE}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
```
...
````markdown
`r ''````{r dog-license-plot, echo=FALSE}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
`r ''````
````
...
````markdown
`r ''````{r dog-license-plot, eval=FALSE}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
`r ''````
````
...
````markdown
`r ''````{r dog-license-plot, fig.cap="Add your figure title here.", fig.height=8, fig.width=8}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
`r ''````
````

```{r dog-license-plot-1, fig.cap="Add your figure title here.", fig.height=8, fig.width=8, echo=FALSE}
ggplot(dog_licenses, aes(x = ..., y = ...)) +
    geom_point()
```

What if we want to first run some code before running the plot, but don't want to 
include the other code? Don't let it get evaluated so it outputs something. So,
this will output the data frame and the plot:

````markdown
`r ''````{r dog-license-plot-after-2000, fig.cap="Add your figure title here.", fig.height=8, fig.width=8}
dog_licenses_after_2000 <- dog_licenses %>% 
    fitler(... >= 2000)
dog_licenses_after_2000
ggplot(dog_licenses_after_2000, aes(x = ..., y = ...)) +
    geom_point()
`r ''````
````

```{r dog-license-plot-after-2000, fig.cap="Add your figure title here.", fig.height=8, fig.width=8, echo=FALSE}
dog_licenses_after_2000 <- dog_licenses %>% 
    fitler(... >= 2000)
dog_licenses_after_2000
ggplot(dog_licenses_after_2000, aes(x = ..., y = ...)) +
...
While this will only output the plot. See how we don't include code that would 
send anything to the console to be "printed"? Only things that get output get
included in the R Markdown output document. Also notice how we renamed the chunk
label? In R Markdown you can't have duplicate code chunk labels.
...
(ref:dog-plot-2000) Add your figure title here.

```{r dog-license-plot-after-2000, fig.cap="(ref:dog-plot-2000)", fig.height=8, fig.width=8, echo=FALSE}
dog_licenses_after_2000 <- dog_licenses %>% 
    fitler(... >= 2000)
ggplot(dog_licenses_after_2000, aes(x = ..., y = ...)) +
    geom_point()
...
Let's create a table of ...

TODO: Finish this once data has been added.
...
If you want to simply print out a number or other text:

````markdown
`r ''````{r numbers-text}
mean(dog_license$..., na.rm = TRUE)
names(dog_license)
`r ''````
````

```{r numbers-text, echo=FALSE}
mean(dog_license$..., na.rm = TRUE)
names(dog_license)
```
...
file. So in our case, the `.bib` file is in the `doc/` folder. Since all
references are appended to the bottom of the document, it's good to add a final
"Reference" section header to the end of your file, like so:
...
This works for PDF, but *not* Word documents.

#### Exercise: Add a summary table, inline results, and a prettier theme

1. Using the R Markdown file from the previous exercise (`using-rmarkdown.Rmd`)
...
TODO: Move this stuff to be included throughout, rather than at end?
...
TODO: Where to put references to original material?
