...
our main work safe from experimental changes.
To do this, we can use branches to work on separate tasks in parallel.

We didn't dwell on it earlier,
but the default branch created when initializing a repository is called `master`.
It is often considered to be the "clean" or "working" version of a
repository's code.
...
for both the bash and python analysis.
...
using commit hashes or `HEAD` and the filename (`git checkout HEAD <file>`).
The `checkout` command can also be used to checkout an entire previous version of the repository,
updating all files in the repository to match the state of a desired commit.

Branches allow us to do this using a human-readable name rather than memorizing a commit hash.
This name also typically gives purpose to the set of changes in that branch.
When we use the command `git checkout <branch_name>`, we are using
...
> **Recap: Step-by-step guide to branching**
>
> 1.  `git checkout master` to make sure you're working in the `master` branch.
>
> 2.  `git pull origin master` to make sure that your starting point is up to date
>
> 3.  `git checkout -b name-of-feature` to create a new branch. The branch's name should be as descriptive. 
>
...
Each person will add a new file with information about a new country in it.

To start, the instructor will now add a single file to the repository containing 
information about the United States.
Next, we will update our local version of the repo to include this new file.
...
Now your local version of the repo is updated but your forked version of the 
repo on GitHub is not yet up to date.
You can reload your fork in GitHub and see it does not contain the new 
`united_states.txt` file.
To update it, we can `push` the changes in our local version to the master branch of our fork, 
called 'origin'.
...
First, let's make a new branch to work on.
This will keep our 'master' version in sync with the authoritative version of the repository.
We can name our branch descriptively after the country we will be adding.
(As an example, these lesson notes will `addFrance`.)
...
Next, copy `united_states.txt` and change the name to the name of your chosen country.
Then you can use a text editor of your choice to edit the contents
of the file to reflect the information of your chosen country.  
...
Now we are ready to submit a pull request.
In other words, we are ready to submit a **request** for a member of the upstream repository
to **pull**  our changes into the upstream repository from our fork.
Once the pull request has been initiated,
they can make comments on our proposed changes and/or request further edits/improvements
before accepting or rejecting our changes.

In order to create this pull request,
we must push our new branch containing the changes we'd like to submit
to the remote linked to our fork, `origin`, on GitHub.
...
> **Recap: Step-by-step guide to pull requests**
>
> 1.  "Fork" the repository that you'd like to submit a pull request to
>     by clicking the button on the GitHub (or equivalent) page for that repository
>
> 2.  `git clone` the fork to your local computer
> 
> 3.  `git remote add upstream` to link your fork with the authoritative repository
>     (i.e. the original GitHub repository that you will submit the pull request to)
>
> 4.  `git checkout -b new-branch` to create a new branch
>
> 5.  Make your proposed edits/changes on that new branch  
>
> 6.  `git push origin new-branch` to push your changes to the fork on GitHub
>
> 7.  Navigate to the GitHub page for your fork and view the new branch.
>     The will be a "compare and pull request" button you can click to create 
