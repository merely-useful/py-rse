...
print("Hello, Zipf!")
```
at the end of `zipf.py`, what happens when you start python interactively and
import the module with the command `import zipf`?
...
1. Turn the `use.py` script into a `pytest` unit test.

## How do I create a package? {#py-package-creating}
...
With that done, we're going to 

<!-- src="package/03b/zipf/generate.py" -->
```python
...
This looks pretty good; we'll just update our test file

<!-- src="package/03b/test_zipf.py" -->
```python
import zipf
...
## What does it mean to install a python package? {#py-package-install}
