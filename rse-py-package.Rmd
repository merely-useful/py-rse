...
which will each be part of the file.  We'll split our routines up into functions that
...
FIXME: Right now torn between using Sphinx since it's defacto standard for Python packages and allows autodocs, but don't wanting to teach rst.  The result is extra complexity that I'm not sure I want...
...
here that is not reflected in our `requirements.txt`; the `Sphinx` and `m2r`
packages.  It isn't needed for running, developing, or even testing
...
the package to have `Sphinx` (which is a large package) and `m2r` 
...
Once this is done, you can import your repository; in this case since there
are additional requirements (the `m2r` package), you will have to make sure
Read The Docs installs this when building the documentation; when importing,
under "Advanced Settings", set the "Requirements file" to be `requirements_docs.txt`.
...
FIXME: Should we just use the configuration file approach here rather than going
into advanced settings?

As with continuous integration testing, there are badges for your README linking
...
```text
[![Documentation Status](https://readthedocs.org/projects/pterry_zipfpy/badge/?version=latest)](https://pterry_zipfpy.readthedocs.io/en/latest/?badge=latest)
```

FIXME: talk about the webhooks, bringing in the discussion from the CI section.
