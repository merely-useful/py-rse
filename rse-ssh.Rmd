# Working Remotely {#rse-ssh}

```{r rse-ssh-setup, include=FALSE}
source("_common.R")
```
...
a [remote login server][remote-login-server] and we will
...
## How did we get here? {#rse-ssh-history}
...
## How can I log in to a remote computer? {#rse-ssh-login}
...
## How can I copy files to and from a remote computer? {#rse-ssh-cp}
...
## How can I run commands on a remote computer? {#rse-ssh-cmd}
...
## How can I create keys so that I don't have to keep typing my password? {#rse-ssh-keys}
...
## What directories and files does SSH depend on? {#rse-ssh-filedir}
...
```shell
$ ssh amira@comet "mkdir ~/.ssh; ls -ld ~/.ssh"
Password: ********
```
