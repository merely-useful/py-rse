...
:   A software tool that extracts specially-formatted comments or [docstrings](#docstring) from code and generates cross-referenced developer documentation.
...
:   The part of the filename that doesn't include the [extension](#filename_extension). For example, the stem of `glossary.yml` is `glossary`.
...
:   In a function, the ability to take any number of arguments. R uses `...` to capture the "extra" arguments. Python uses `*args` and `**kwargs` to capture unnamed, and named, "extra" arguments, respectively.  See also: [keyword arguments](#keyword_arguments)
