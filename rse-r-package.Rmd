...
a way to build and combine software libraries.
Every widely-used language now has an online repository
from which people can download and install such libraries,
...
-   Just as `.gitignore` tells Git what files in a project to ignore,
    `.Rbuildignore` tells RStudio which files *shouldn't* be included in the package.
...
-   Manual pages go in the `man` directory.
    ...
    The good news is,
    we can embed comments in our source code and use a tool called [roxygen2][roxygen2]
...
What are the pros and cons of distributing packages as source code, byte code, and binary code?
...
select "New Project",
and then tell RStudio to make it a package (and a Git repository):
...
and then make the following changes to `DESCRIPTION`:

1.  change the `Title`, `Author`, `Maintainer`, and `Description`;
2.  change the `License` to [`MIT`][mit-license] (see FIXME for other options); and
...
```
── R CMD check results ───────────────────────────────────── zipffreq 0.1.0 ────
Duration: 11.6s
...
The explanations for these are:

1.  We haven't written any documentation for the `count_words` function.
2.  We haven't described our license in the approved way.
3.  Three of the files we added for GitHub's benefit have to be renamed or moved.

After a bit of searching online,
we move `LICENSE.md` to `LICENSE` with no extension (but still in the project's root directory)
and change the entry in `DESCRIPTION` for the license to `License: MIT + file LICENSE` (spelled exactly that way).
...
we discover that we need to load the `devtools` package in the console
...
Next,
we load the `usethis` package with `library(usethis)`
and called `usethis::use_data(small_data)`
