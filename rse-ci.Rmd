...
```{r rse-ci-setup, include=FALSE}
source("_common.R")
```

## Introduction {#rse-ci-intro}

[Continuous integration][continuous-integration] (CI) in software
...
```{r image-add-repo, echo=FALSE, fig.cap="ci-add-repo"}
...
```{r image-list-repos, echo=FALSE, fig.cap="ci-list-repos"}
...
contains [YAML][yaml] key-value settings that Travis will use and
...
called `requirements.txt` for `pip` to use (Chapter \@ref(rse-package-py)).
...
Figure \@ref(fig:image-github-icon). This will bring up a dialog box where you can select the item
...
your project will now show the "badge" for the Travis CI build as shown in
Figure \@ref(fig:image-github-icon).

```{r image-github-icon, echo=FALSE, fig.cap="The Travis Build Badge"}
...
FIXME: Reference to unit test chapter.
...
FIXME: Do we want to add more explanation about this? I feel running unit tests are basically the biggest reason to use CI.

## Using CI to test coverage {#rse-ci-coverage}

We can also add code coverage,
which is in Section \@ref(rse-correct-coverage),
...
To send the code to FIXME: finish this paragraph.
...
website for your R package, we can run:
...
This sets up the necessary infrastructure to create a website from your package
...
FIXME: Add information for Python projects
