...
development.
3. CI is particularly useful when working on a project with others, as bugs that arise from someone else's code contribution can be harder for you to resolve so preventing them from being merged in the first place is even more time saving than when working alone. By encouraging frequent well-teseted merges, continuous integration also alleviates the problems that can occur when multiple people wait to integrate their work until just before a big milestone and then want to merge many big changes all at once.
...
Idea:
- Get them to get their own project to use travis, assuming that they will be
working on a single package throughout the whole course.
- Write a unit test that you know will fail. Push the change up to GitHub and
explore the log of the Travis build to see how Travis displays your errors.
- Correct the test, push to GitHub, and get Travis to build successfully.
