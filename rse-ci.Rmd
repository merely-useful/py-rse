...
>
> This process is for general Travis usage.
...
```{r rse-ci-add-repo, echo=FALSE, fig.cap="ci-add-repo"}
knitr::include_graphics("figures/rse-ci/add-repo.png")
```

```{r rse-ci-list-repos, echo=FALSE, fig.cap="ci-list-repos"}
...
-   `language` tells Travis which programming language we are using
...
-   Since we are using Python,
    we provide some information about the version we want under the `python` key.
    We can ask Travis to test our project with several different versions
    by adding more versions to the list.
-   `install` tells Travis how to install the software we need for our project.
    For Python projects,
...
-   Finally,
    the entries under the `script` key tell Travis what to do.
...
Travis' summary report tells us whether the build passed,
...
```{r rse-ci-build-overview, echo=FALSE, fig.cap="Travis Build Overview"}
...
```{r rse-ci-github-icon, echo=FALSE, fig.cap="The Travis CI Build Badge"}
...
FIXME: Include examples here of failing tests in R.
...
FIXME: Include examples here of failing tests in Python.
...
FIXME: this section needs to be filled in - as it is, readers will know something is possible
