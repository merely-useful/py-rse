...
![dog license data in a spreadsheet program](figures/dog_licenses_spreadsheet_screenshot.png)
...
# here's what the columns look like in camel case
dog_licenses.columns
```
...
# here's what the columns now look like in snake case
dog_licenses.columns
```
...
large_borough_names = borough_names[dog_licenses['borough'].value_counts() >= 5]
...
4. **Creating new columns**
  ...
  2. Create a new data frame from the dog license data frame `dog_licenses` that contains only the `animal_birth_month`, `animal_name`, and `breed_name` columns.
